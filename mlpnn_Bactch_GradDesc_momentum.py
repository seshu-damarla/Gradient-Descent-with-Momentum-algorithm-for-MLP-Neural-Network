...
def initialization(nh, n_inputs):
    ...
    return wh, bh, wo, bo, Vwh, Vwo, Vbh, Vbo
...
def backward_proagation_gradients(m, wh, bh, wo, bo, Vwh, Vwo, Vbh, Vbo, xtrain, ytrain, beta):
    sum1=0
    sum2=0
    sum3=0
    sum4=0
...
#        print(ei.shape)
    ...
    dwh=(-sum1)/m
    Vwh = beta*Vwh+ (1-beta)*dwh
    dbh=(-sum2)/m 
    Vbh = beta*Vbh + (1-beta)*dbh
    dwo=(-sum3)/m
    Vwo= beta*Vwo + (1-beta)*dwo
    dbo=(-sum4)/m
    Vbo = beta*Vbo + (1-beta)*dbo
    
    return Vwh, Vbh, Vwo, Vbo

def mlp_model(xtrain, ytrain, nh, learning_rate, epochs, beta):
    n_inputs = xtrain.shape[1]  # no. of features of the input smaples
    m = xtrain.shape[0]         # no. of training examples
    wh, bh, wo, bo, Vwh, Vwo, Vbh, Vbo = initialization(nh, n_inputs)
    J=[]
    for i in range(epochs):
        lossfn = forward_propgation_lossfn(m, wh, bh, wo, bo, xtrain, ytrain)
        dwh, dbh, dwo, dbo = backward_proagation_gradients(m, wh, bh, wo, bo, Vwh, Vwo, Vbh, Vbo, xtrain, ytrain, beta)
...
beta = 0.8
#print(ytrain.shape)
wh, bh, wo, bo, J = mlp_model(xtrain, ytrain, nh, learning_rate, epochs, beta)
